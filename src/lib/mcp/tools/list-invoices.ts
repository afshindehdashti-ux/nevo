...
export default withAudit({
  ...
  inputSchema: {
    status: z
      ...
      .trim()
      .optional()
      .describe("Optional status filter (e.g. draft, sent, paid)."),
  ...
  handler: async (
    ...
    let q = supabaseForUser(ctx)
      .from("invoices")
      .select(
        "id, invoice_number, customer_id, status, subtotal, tax_amount, total, currency, issue_date, due_date, created_at",
      )
