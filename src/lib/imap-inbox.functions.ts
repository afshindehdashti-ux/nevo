...
export const saveMailboxConfig = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((data: SaveMailboxConfigInput) => saveConfigSchema.parse(data))
  ...
  .handler(async ({ context }) => {
    ...
    try {
      ...
      try {
        ...
        // Best-effort logout; the primary operation result is already known.
      }
    }
  });
...
export const listImapMessages = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((data: z.infer<typeof listSchema>) => listSchema.parse(data ?? {}))
  .handler(async ({ data, context }) => {
    await ensureSuperAdmin(context.supabase, context.userId);
    const row = await loadActiveConfig(context.supabase);
...
export const getImapMessage = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((data: z.infer<typeof getSchema>) => getSchema.parse(data))
  .handler(async ({ data, context }) => {
    await ensureSuperAdmin(context.supabase, context.userId);
    const row = await loadActiveConfig(context.supabase);
    const mailbox = data.mailbox || "INBOX";
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Marking as read is best-effort and must not block message display.
        }
...
export const setImapSeen = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((d: z.infer<typeof flagSchema>) => flagSchema.parse(d))
  .handler(async ({ data, context }) => {
    await ensureSuperAdmin(context.supabase, context.userId);
    const row = await loadActiveConfig(context.supabase);
    const mailbox = data.mailbox || "INBOX";
...
export const deleteImapMessage = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((d: z.infer<typeof deleteSchema>) => deleteSchema.parse(d))
  .handler(async ({ data, context }) => {
    await ensureSuperAdmin(context.supabase, context.userId);
    const row = await loadActiveConfig(context.supabase);
    const mailbox = data.mailbox || "INBOX";
...
export const startGmailOAuth = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((d: z.infer<typeof startOAuthSchema>) => startOAuthSchema.parse(d))
  .handler(async ({ data, context }) => {
    await ensureSuperAdmin(context.supabase, context.userId);
