...
export type EnqueueResult =
  ...
  | { ok: false; reason: 'email_suppressed' | 'unknown_template' | 'render_failed' | 'enqueue_failed' | 'config_error' | 'suppression_check_failed'; message?: string }
...
export async function enqueueTransactionalEmail(opts: EnqueueOptions): Promise<EnqueueResult> {
  ...
  if (!template) {
    return { ok: false, reason: 'unknown_template', message: `Template '${opts.templateName}' not registered` }
  }
