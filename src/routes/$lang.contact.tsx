...
function ContactPage() {
  ...
  const offices = (
    t("contact.offices.list", { returnObjects: true }) as Array<{
      city: string;
      country: string;
      role: string;
      address: string;
  ...
  const cards = t("contact.cards.items", { returnObjects: true }) as Array<{
    t: string;
    d: string;
    v?: string;
  }>;
  ...
  return (
    <>
        ...
        <div className="container-wide relative py-32 md:py-40">
          ...
          >
            ...
            <h1 className="mt-4 max-w-4xl text-5xl font-semibold tracking-tight md:text-7xl">
              {t("contact.hero.titleA")}{" "}
              <span className="text-emerald-400">{t("contact.hero.titleB")}</span>
            </h1>
            <p className="mt-6 max-w-2xl text-lg text-white/70">{t("contact.hero.lede")}</p>
            <div className="mt-10 flex flex-wrap gap-3">
              <Button size="lg" asChild>
                <a href="#offices">
                  {t("contact.hero.ctaOffices")} <ArrowRight className="ml-2 h-4 w-4" />
                </a>
              </Button>
              <Button
                size="lg"
            ...
            <div className="mt-16 grid max-w-2xl grid-cols-3 gap-8 border-t border-white/10 pt-8">
              <div>
                <div className="text-4xl font-semibold text-emerald-400">{offCount}</div>
                <div className="mt-1 text-xs uppercase tracking-widest text-white/50">
                  {t("contact.hero.statOffices")}
                </div>
              </div>
              <div>
                <div className="text-4xl font-semibold text-emerald-400">{countries}+</div>
                <div className="mt-1 text-xs uppercase tracking-widest text-white/50">
                  {t("contact.hero.statCountries")}
                </div>
              </div>
              <div>
                <div className="text-4xl font-semibold text-emerald-400">{engineers}+</div>
                <div className="mt-1 text-xs uppercase tracking-widest text-white/50">
                  {t("contact.hero.statEngineers")}
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <Section>
        <SectionHeader
          eyebrow={t("contact.cards.eyebrow")}
          title={t("contact.cards.title")}
          lede={t("contact.cards.lede")}
        />
      ...
      <Section tone="surface" id="request-callback">
        <div className="grid gap-10 lg:grid-cols-2">
          <div className="overflow-hidden rounded-2xl border border-border">
            <iframe
              title={t("contact.callback.mapTitle")}
          ...
          <div className="rounded-2xl border border-border bg-background p-8 md:p-10">
            ...
            <form ref={formRef} className="mt-8 grid gap-4" onSubmit={handleCallback} noValidate>
              <div className="grid gap-4 md:grid-cols-2">
                <input
                  name="name"
                  placeholder={t("contact.callback.name")}
                  required
                  className="rounded-md border border-input bg-background px-4 py-3 text-sm"
                />
                <input
                  name="company"
                  placeholder={t("contact.callback.company")}
                  className="rounded-md border border-input bg-background px-4 py-3 text-sm"
                />
                <input
                  name="email"
                  placeholder={t("contact.callback.email")}
                  type="email"
                  required
                  className="rounded-md border border-input bg-background px-4 py-3 text-sm"
                />
              ...
              >
                <option value="" disabled>
                  {t("contact.callback.interestPlaceholder")}
                </option>
              ...
              <textarea
                name="message"
                rows={4}
                placeholder={t("contact.callback.message")}
                className="rounded-md border border-input bg-background px-4 py-3 text-sm"
              />
              <Button type="submit" size="lg" disabled={busy}>
                {busy ? (
                  ...
                  <>
                    {t("contact.callback.submit")} <ArrowRight className="ml-2 h-4 w-4" />
                  </>
                )}
              </Button>
            </form>
