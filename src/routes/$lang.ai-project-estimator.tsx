...
import {
  Sparkles, MapPin, Gauge, Layers, Cog, Factory, Wallet, ChevronRight,
  Download, ArrowRight, Zap, Droplet, Wind, Flame, Users, Truck,
  TrendingUp, DollarSign, Clock, ShieldCheck, CheckCircle2, BarChart3,
  PhoneCall, MessageCircle, Mail, Brain, Target, Cpu, Building2,
  ClipboardList, Wrench, Rocket, RefreshCw,
} from "lucide-react";
...
const FAQS = [
  { q: "How accurate is the AI Project Estimator?", a: "The estimator uses real market data from 1,500+ completed NEVO projects across 85+ countries. Typical accuracy is ±8% at concept stage — enough to make a confident go/no-go investment decision before commissioning a full feasibility study." },
  { q: "What panel technology should I choose?", a: "PIR offers the best thermal performance and highest ROI for cold storage and industrial cladding. Rock Wool leads on fire performance. PUR is cost-optimized for high-volume production. EPS is entry-level. Hybrid lines can switch between cores on demand." },
  { q: "Can the AI recommend a factory layout?", a: "Yes. Based on capacity, panel technology and factory type, the engine proposes zoning for raw material warehouse, roll-forming, foaming, laminator, cutting, stacking, packaging, finished goods, offices and utilities." },
  { q: "How is ROI calculated?", a: "ROI, IRR and payback are modeled from CAPEX, OPEX, capacity utilization, regional pricing and 260 working-days/year. The 5-year model assumes progressive ramp-up (60% Y1 → 95% Y5)." },
  { q: "What happens after I generate an estimation?", a: "You can request a full engineering proposal. NEVO engineers refine the model with your exact site conditions, utilities availability and regulatory constraints — usually within 5 business days." },
];
  ...
  head: () => ({
    ...
    scripts: [
      ...
      { type: "application/ld+json", children: JSON.stringify(breadcrumbJsonLd([
        { name: "Home", path: "/" }, { name: "AI Project Estimator", path: "/ai-project-estimator" },
      ])) },
...
const COUNTRIES: Country[] = [
  { code: "SA", name: "Saudi Arabia", flag: "🇸🇦", costIdx: 1.00, revIdx: 1.00 },
  ...
  { code: "ZA", name: "South Africa", flag: "🇿🇦", costIdx: 0.90, revIdx: 0.84 },
...
const PANELS = [
  { key: "PIR",       density: 40, costK: 1.00, revK: 1.10, thermal: 5, fire: 3, roi: 5 },
  ...
  { key: "Hybrid",    density: 60, costK: 1.20, revK: 1.15, thermal: 4, fire: 4, roi: 5 },
] as const;
type PanelKey = typeof PANELS[number]["key"];

const AUTOMATION = [
  { key: "Semi Automatic", costK: 0.78, opexK: 1.12, opFactor: 1.30 },
  { key: "Automatic",       costK: 1.00, opexK: 1.00, opFactor: 1.00 },
...
const FACTORY_TYPE_K: Record<FactoryType, number> = { "New Factory": 1.00, "Expansion": 0.62, "Upgrade": 0.35, "Turnkey": 1.15 };
...
/* ─── Engine ─── */
function estimate(country: Country, cap: Capacity, panel: PanelKey, auto: AutomationKey, ftype: FactoryType) {
  ...
  const steam = Math.round(cap * 0.30 + 100);         // kg/h
  ...
  const risk = cap >= 15000 && ftype === "New Factory" ? "Moderate"
             : cap <= 3000 && auto === "Fully Automatic" ? "Moderate"
             : "Low";

  return {
    capex, buildingArea, landArea, productionArea, warehouseArea, officeArea,
    power, waterDaily, air, steam, operators, forklifts,
    annualM2, revenue, opex, gross, net, roi, irr, payback,
    linesCount, perLine, utilization: 85, risk,
    workingDays,
  };
}
...
function EstimatorPage() {
  ...
  const result = useMemo(() => estimate(country, capacity, panel, auto, ftype), [country, capacity, panel, auto, ftype]);

  const generate = () => {
    setBusy(true);
    setTimeout(() => { setBusy(false); setGenerated(true); setTimeout(() => {
    ...
    <div className="min-h-screen bg-[#050807] text-white/90">
          ...
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-6 text-sm">
              {[
                ...
                <div key={t} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
                  <Icon className="h-4 w-4 text-emerald-300" />
                  <div><div className="text-xs font-semibold text-white">{t}</div><div className="text-[11px] text-white/50">{s}</div></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
              <div className="mb-4 flex items-center justify-between">
                ...
                <button
                  onClick={() => { setCountry(COUNTRIES[0]); setCapacity(8000); setPanel("PIR"); setAuto("Automatic"); setFtype("New Factory"); setBudget("5–10M USD"); }}
                  className="inline-flex items-center gap-1 text-[11px] text-white/50 hover:text-white/80"
                ><RefreshCw className="h-3 w-3" /> Reset</button>
              </div>
              ...
              <StepShell n={1} label="Project Country" icon={MapPin}>
                ...
                >
                  {COUNTRIES.map(c => <option key={c.code} value={c.code} className="bg-[#0a0d0c]">{c.flag} {c.name}</option>)}
                </select>
              </StepShell>
              ...
              <StepShell n={2} label="Production Capacity" icon={Gauge}>
                <select
                  ...
                  className="w-full appearance-none rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-white outline-none focus:border-emerald-500/50"
                >
                  {CAPACITIES.map(c => <option key={c} value={c} className="bg-[#0a0d0c]">{fmt(c)} m² / day</option>)}
                </select>
              </StepShell>
              ...
              <button onClick={generate} disabled={busy}
                ...
                {busy ? <><RefreshCw className="h-4 w-4 animate-spin" /> Analyzing…</> : <>Generate AI Estimation <ArrowRight className="h-4 w-4" /></>}
              </button>
              <div className="mt-3 flex items-start gap-2 rounded-xl border border-white/5 bg-white/[0.02] p-3 text-[11px] text-white/50">
                ...
                Our AI engine will analyze your inputs and generate a comprehensive project estimation.
              </div>
            </div>
          </aside>
          ...
          <div id="ai-results" className="min-w-0 space-y-6">
            <AnimatePresence mode="wait">
              {!generated ? (
                ...
                <motion.div key="live" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <section className="border-t border-white/5 bg-[#070a09]">
        <div className="mx-auto max-w-[1440px] px-6 py-16">
          <div className="mb-6 text-xs uppercase tracking-[0.2em] text-emerald-300/90">Frequently asked</div>
          <h2 className="text-3xl font-semibold md:text-4xl">Answers before you build.</h2>
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {FAQS.map((f, i) => (
              <details key={i} className="group rounded-2xl border border-white/10 bg-white/[0.03] p-5 open:bg-white/[0.05]">
      ...
      <section className="relative border-t border-white/5 bg-gradient-to-b from-[#050807] to-[#0a1210]">
        <div className="mx-auto max-w-[1440px] px-6 py-16 md:py-20">
          <div className="grid gap-8 md:grid-cols-[1.4fr_1fr] md:items-center">
            <div>
              <div className="text-xs uppercase tracking-[0.2em] text-emerald-300/90">Ready to build your factory?</div>
              <h2 className="mt-3 text-3xl font-semibold md:text-5xl">
                Turn this estimation into a <span className="text-emerald-400">detailed engineering proposal</span>.
              </h2>
              <p className="mt-4 max-w-xl text-white/70">
                NEVO engineers will refine every parameter with your site conditions and deliver a full turnkey plan — usually within 5 business days.
              </p>
            </div>
      ...
      <div className="fixed inset-x-3 bottom-3 z-40 lg:hidden">
        <Link to="/project-inquiry" className="flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-black shadow-2xl shadow-emerald-500/30">
  ...
  return (
    <div className={`${last ? "" : "mb-3 border-b border-white/5 pb-3"}`}>
      <div className="mb-2 flex items-center gap-2 text-[11px] uppercase tracking-wider text-white/60">
        <span className="grid h-5 w-5 place-items-center rounded-full bg-emerald-500/15 text-[10px] font-bold text-emerald-300 ring-1 ring-emerald-500/30">{n}</span>
...
function PreEstimate({ onGenerate, busy }: { onGenerate: () => void; busy: boolean }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-10 text-center">
      ...
      <h3 className="mt-5 text-2xl font-semibold">Configure your project. Get instant AI feasibility.</h3>
      <p className="mx-auto mt-2 max-w-xl text-sm text-white/60">
        Answer six quick questions on the left. Our engineering AI will produce investment, utilities,
        capacity, ROI, IRR and payback in seconds — with a downloadable report.
      </p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-emerald-300/90">
            ...
            <span className="ml-2 rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-300 ring-1 ring-emerald-500/30">Generated in 8.7 sec</span>
          </div>
          <div className="mt-1 text-sm text-white/60">{country.flag} {country.name} · {fmt(capacity)} m²/day · {panel} · {auto}</div>
        </div>
...
function KpiRow({ r }: any) {
  const items = [
    ...
    { l: "Annual Production", v: `${(r.annualM2/1_000_000).toFixed(2)}M m²`, s: `${r.workingDays} working days`, i: Factory },
    { l: "ROI (5 Years)", v: `${(r.roi*3.2).toFixed(1)}%`, s: r.roi > 20 ? "Excellent" : r.roi > 12 ? "Good" : "Fair", i: TrendingUp, tone: "emerald" as const },
    { l: "Payback Period", v: `${Math.max(1.5, r.payback).toFixed(1)} Years`, s: r.payback < 4 ? "Very Good" : "Good", i: Clock },
    { l: "IRR (10 Years)", v: `${r.irr}%`, s: r.irr > 25 ? "Excellent" : "Good", i: BarChart3, tone: "emerald" as const },
  ];
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      {items.map(it => (
        <div key={it.l} className={`rounded-2xl border p-4 ${it.tone === "emerald" ? "border-emerald-500/30 bg-emerald-500/[0.05]" : "border-white/10 bg-white/[0.03]"}`}>
          ...
          <div className={`mt-1 text-[11px] ${it.tone === "emerald" ? "text-emerald-300" : "text-white/50"}`}>{it.s}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {rows.map(({ i: Icon, l, v }) => (
          <div key={l} className="flex items-start gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-3">
...
function FinancialCharts({ r }: any) {
  ...
  const cum: number[] = []; net.reduce((a,v,i) => { const s=a-(i===0?r.capex/1_000_000:0)+v; cum.push(+s.toFixed(1)); return s; }, 0);

  return (
    <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
      <ChartCard title="Cash Flow (USD M)">
        <BarBiChart inflows={inflows} outflows={outflows} labels={yrs.map(y=>`Y${y}`)} />
        <ChartLegend items={[["Inflow","bg-emerald-400"],["Outflow","bg-rose-400"]]} />
      </ChartCard>
      <ChartCard title="ROI Over Time (%)" pill={`ROI Y5 ${roiCum[4]}%`}>
        <LineChart values={roiCum} labels={yrs.map(y=>`Y${y}`)} suffix="%" />
      </ChartCard>
      <ChartCard title="Investment Recovery (USD M)" pill={`Payback ${Math.max(1.5, r.payback).toFixed(1)} y`}>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
      <div className="mb-3 flex items-center justify-between text-xs">
        ...
        {pill && <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-300 ring-1 ring-emerald-500/30">{pill}</span>}
      </div>
      {children}
    </div>
  );
...
function ChartLegend({ items }: { items: [string,string][] }) {
  return (
    <div className="mt-2 flex gap-3 text-[10px] text-white/50">
      {items.map(([l,c]) => <span key={l} className="flex items-center gap-1"><span className={`h-2 w-2 rounded-sm ${c}`} />{l}</span>)}
    </div>
  );
}
      ...
      {inflows.map((v: number, i: number) => (
        <div key={i} className="flex flex-1 flex-col items-center gap-1">
          <div className="flex w-full items-end gap-0.5">
            <div className="flex-1 rounded-t bg-emerald-400/80" style={{ height: `${(v/max)*90}px` }} title={`Inflow ${v}M`} />
            <div className="flex-1 rounded-t bg-rose-400/80" style={{ height: `${(outflows[i]/max)*90}px` }} title={`Outflow ${outflows[i]}M`} />
          </div>
...
function LineChart({ values, labels, suffix, showZero }: any) {
  ...
  const path = values.map((v: number, i: number) => `${i===0?"M":"L"} ${i*step} ${y(v)}`).join(" ");
  return (
    <div>
      <svg viewBox={`0 0 ${w} ${h}`} className="h-28 w-full">
        <path d={`${path} L ${w} ${h} L 0 ${h} Z`} fill="url(#lg)" opacity="0.4" />
        <defs><linearGradient id="lg" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#10b981" stopOpacity="0.5"/><stop offset="100%" stopColor="#10b981" stopOpacity="0"/></linearGradient></defs>
        <path d={path} fill="none" stroke="#10b981" strokeWidth="2" />
        {values.map((v: number, i: number) => <circle key={i} cx={i*step} cy={y(v)} r="2.5" fill="#10b981" />)}
      </svg>
    ...
    <div>
      <div className="flex h-28 items-end gap-2">
        {values.map((v: number, i: number) => (
          <div key={i} className="flex-1 rounded-t bg-gradient-to-t from-emerald-500/40 to-emerald-400" style={{ height: `${Math.max(4,(v/max)*100)}%` }} />
        ))}
      </div>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="text-xs text-emerald-300 hover:text-emerald-200">View full equipment list →</button>
      </div>
...
function LayoutAndRecommend({ panel, auto, ftype, r }: any) {
  const zones = [
    "Raw Material Warehouse","Roll Forming Line","Double Belt Laminator","Foaming Area",
    "Cooling Section","Cutting Station","Stacking System","Packaging Area",
    "Finished Goods Warehouse","Loading Bays","Office & Laboratory","Utilities Area",
  ];
    ...
    <div className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex gap-1 rounded-lg border border-white/10 bg-white/[0.03] p-0.5">
            {(["3D","TOP","EXPLODED"] as const).map(v => (
              <button key={v} onClick={() => setView(v)} className={`rounded px-2.5 py-1 text-[10px] font-semibold ${view===v ? "bg-emerald-500 text-black" : "text-white/60 hover:text-white"}`}>{v} VIEW</button>
            ))}
          </div>
        </div>
        <div className="grid gap-3 md:grid-cols-[1.4fr_1fr]">
          ...
          <ul className="space-y-1.5 text-xs">
            {zones.map((z, i) => (
              <li key={z} className="flex items-center gap-2 rounded-lg border border-white/5 bg-white/[0.02] px-2.5 py-1.5">
                <span className="grid h-4 w-4 place-items-center rounded-full bg-emerald-500/15 text-[9px] font-bold text-emerald-300 ring-1 ring-emerald-500/30">{i+1}</span>
      ...
      <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.04] p-5">
        ...
        <div className="mt-4 flex items-center gap-2 text-xs">
          <span className="rounded-full bg-white/5 px-2 py-0.5 text-white/60 ring-1 ring-white/10">Risk: <span className="text-emerald-300">{r.risk}</span></span>
          <span className="rounded-full bg-white/5 px-2 py-0.5 text-white/60 ring-1 ring-white/10">Utilization: <span className="text-emerald-300">{r.utilization}%</span></span>
        </div>
        ...
        <div className="flex items-center gap-6">
          <svg viewBox="0 0 120 120" className="h-40 w-40 -rotate-90">
            ...
            {parts.map((p, i) => {
              ...
              const el = <circle key={i} cx="60" cy="60" r={R} fill="none" stroke={p.c} strokeWidth="14" strokeDasharray={dash} strokeDashoffset={-acc} />;
              acc += len;
              return el;
            })}
          </svg>
          <div className="flex-1 space-y-2 text-xs">
            {parts.map(p => (
              <div key={p.l} className="flex items-center justify-between">
                <span className="flex items-center gap-2 text-white/80"><span className="h-2 w-2 rounded-full" style={{ background: p.c }} />{p.l}</span>
                <span className="font-semibold text-white">{p.p}%</span>
              </div>
            ))}
            <div className="mt-2 border-t border-white/5 pt-2 text-sm">
              <span className="text-white/60">Total</span> <span className="ml-2 font-semibold text-emerald-300">{fmtUSD(capex)}</span>
            </div>
          </div>
        </div>
      </div>
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
        ...
        <ul className="mt-4 grid grid-cols-2 gap-2 text-xs text-white/70">
          {[
            ...
            "Productivity: 95%",
          ].map(t => <li key={t} className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-300" />{t}</li>)}
        </ul>
      </div>
    </div>
  );
...
function ReportPreview() {
  const items = [
    "Executive Summary","Project Overview","Financial Estimation","Technical Specification",
    "Utilities & Requirements","Equipment List","Factory Layout","Risk Analysis","Next Steps",
  ];
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
        ...
        <ul className="grid grid-cols-1 gap-2 text-sm">
          {items.map(t => (
            <li key={t} className="flex items-center gap-3 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2.5 text-white/80">
