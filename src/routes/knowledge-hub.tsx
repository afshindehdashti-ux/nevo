...
import {
  ...
  FileText, Wrench, Factory, FlaskConical, Layers, Flame, Shield,
  Snowflake, Building2, ClipboardCheck, Sparkles, Calculator, Rss,
  PlayCircle, Download, Clock, Star, TrendingUp, HelpCircle, Mail,
} from "lucide-react";
    ...
    scripts: [
      ...
      },
      {
        ...
        children: JSON.stringify({
          ...
          description: "Engineering knowledge library for sandwich panel technology, factory development and production.",
          url: "/knowledge-hub",
        }),
      },
      {
...
const CATEGORIES: (Category | "All")[] = [
  "All","PIR","PUR","Rock Wool","EPS","Cold Rooms","Clean Rooms","Fire","Thermal",
  "Production Lines","Factory Design","Steel Coils","Chemicals","Automation","Quality","Project Management",
];

const COURSES = [
  { level: "Beginner",     title: "Sandwich Panels — Foundations",              hours: 6,  lessons: 18, cover: k17, tag: "Free" },
  { level: "Beginner",     title: "Reading a Panel Datasheet",                  hours: 3,  lessons: 10, cover: k01, tag: "Free" },
  { level: "Professional", title: "PIR Line Operation Essentials",              hours: 12, lessons: 28, cover: k07, tag: "Pro" },
  { level: "Professional", title: "Cold Room Engineering Masterclass",          hours: 10, lessons: 24, cover: k21, tag: "Pro" },
  { level: "Expert",       title: "Factory Investment & Financial Modeling",    hours: 16, lessons: 32, cover: k36, tag: "Expert" },
  { level: "Expert",       title: "Line Commissioning & FAT/SAT",               hours: 14, lessons: 26, cover: k06, tag: "Expert" },
];
...
const FAQS: { group: string; icon: typeof HelpCircle; items: { q: string; a: string }[] }[] = [
  {
    group: "Technical Questions", icon: FileText, items: [
      { q: "What thickness of panel do I need for a −25 °C freezer?", a: "For −25 °C, a 150–180 mm PIR panel typically reaches U ≈ 0.14 W/m²K, meeting energy targets in most climates. Use our Panel Thickness Calculator to size for local conditions." },
      { q: "What's the difference between PIR and PUR?", a: "PUR is a polyurethane rigid foam; PIR is polyisocyanurate — a modified PUR with higher aromatic content that improves fire performance (typically B-s1,d0) while retaining a low λ ≈ 0.022 W/mK." },
      { q: "How is U-value calculated?", a: "U = λ / d, where λ is thermal conductivity (W/mK) and d is core thickness in metres. Skins and interfaces add small resistances; EN 14509 gives the full method." },
    ],
  },
  {
    group: "Production Questions", icon: Factory, items: [
      { q: "What is a typical line speed?", a: "Continuous PIR lines run 6–15 m/min depending on thickness and panel type. Rock wool lines are slower — typically 3–8 m/min due to lamella insertion." },
      { q: "How many people run a modern sandwich panel line?", a: "A fully automated NEVO line runs with 6–8 operators per shift, plus quality, maintenance and warehouse staff." },
      { q: "What OEE should I target?", a: "World-class continuous lines reach 82–88% OEE. New factories typically start at 60–70% and improve over 12–18 months with SPC and TPM programmes." },
    ],
  },
  {
    group: "Investment Questions", icon: TrendingUp, items: [
      { q: "How much does a sandwich panel factory cost?", a: "A 1 M m²/yr continuous PIR factory typically costs 8–12 M USD turnkey (line + building + utilities + working capital), depending on automation level and country." },
      { q: "What payback period is realistic?", a: "Typical payback is 3.5–5 years at 35–45% gross margin. Our Investment Calculator models CAPEX, OPEX, IRR and NPV for your inputs." },
      { q: "Can the factory be expanded in phases?", a: "Yes — plan a second line at Year 3 with ~55% incremental capex. Design the building and utilities for Phase II from day one." },
    ],
  },
  {
    group: "Engineering Questions", icon: Wrench, items: [
      { q: "Which fire standard applies in my country?", a: "Most markets recognise EN 13501-1 (EU) or ASTM E84 (US). Petrochemical projects add EI-integrity per EN 1364-1. Ask our engineers for a country-specific reference." },
      { q: "What is the difference between roof and wall panels?", a: "Roof panels use 5-rib trapezoidal profiles for drainage and load-span; wall panels use micro-rib or flush profiles for aesthetics. Both use the same core technology." },
    ],
  },
  {
    group: "Installation Questions", icon: ClipboardCheck, items: [
      { q: "How are panels fixed to structure?", a: "Wall panels use hidden or exposed fasteners into steel purlins/rails per panel manufacturer detail. Cold room panels use cam-lock joints for tight thermal seal." },
      { q: "What's the maximum panel length?", a: "Continuous lines produce panels up to 24 m for standard trucks. Special transport (flat-rack containers, break-bulk) enables longer panels for architectural projects." },
    ],
  },
  {
    group: "Maintenance Questions", icon: Shield, items: [
      { q: "How often should facades be inspected?", a: "Annual visual inspection is standard; a full fastener and seal check every 3 years. Coastal environments require 6-month inspections and touch-up of any coating damage." },
      { q: "How long do sandwich panels last?", a: "Well-maintained PVDF-coated panels have a service life of 30–40 years; polyester-coated panels 15–25 years. Cores retain thermal performance for the panel's full life." },
    ],
  },
];
    ...
    <div className="min-h-screen bg-[#05070a] text-white">
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-px bg-white/5 border border-white/5 rounded-2xl overflow-hidden max-w-3xl">
            {[
              ...
              <div key={m.v} className="bg-[#0a0d10] p-5">
                ...
                <div className="mt-1 text-[10px] font-mono uppercase tracking-widest text-white/50">{m.v}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-b border-white/5 py-20">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-[10px] font-mono uppercase tracking-widest text-emerald-400">Explore</div>
          <h2 className="mt-2 text-3xl font-semibold tracking-tight">14 knowledge sections</h2>
          <p className="mt-3 text-white/60 max-w-2xl">From beginner foundations to expert-level factory financial modeling — organized by discipline.</p>
            ...
            {SECTIONS.map((s) => {
              ...
              const inner = (
                <div className="group h-full rounded-2xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] hover:border-emerald-500/30 p-5 transition">
                  ...
                  <div className="mt-4 font-medium group-hover:text-emerald-300 transition">{s.name}</div>
                </div>
              );
              return isExternal ? <Link key={s.name} to={s.href}>{inner}</Link> : <a key={s.name} href={s.href}>{inner}</a>;
            })}
          </div>
        </div>
      </section>
      ...
      <section id="articles" className="border-b border-white/5 py-20">
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              <div className="text-[10px] font-mono uppercase tracking-widest text-emerald-400">Editorial library</div>
              <h2 className="mt-2 text-3xl font-semibold tracking-tight">Technical articles & engineering guides</h2>
            </div>
            <div className="flex gap-1 rounded-lg border border-white/10 bg-white/[0.03] p-1">
              {(["Featured","Latest","Most Popular"] as Tab[]).map((t) => (
                <button key={t} onClick={() => setTab(t)} className={`px-3 py-1.5 text-xs rounded-md transition ${tab === t ? "bg-emerald-500 text-black font-semibold" : "text-white/70 hover:text-white"}`}>{t}</button>
              ))}
            </div>
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {CATEGORIES.map((c) => (
              <button key={c} onClick={() => setCat(c)} className={`rounded-full px-3 py-1.5 text-xs transition ${cat === c ? "bg-emerald-500 text-black font-semibold" : "border border-white/10 bg-white/[0.03] text-white/70 hover:text-white"}`}>{c}</button>
            ))}
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {(["All","Beginner","Professional","Expert"] as Level[]).map((l) => (
              <button key={l} onClick={() => setLevel(l)} className={`rounded-md px-3 py-1.5 text-[11px] font-mono uppercase tracking-widest transition ${level === l ? "bg-white/10 text-white border border-white/20" : "border border-white/10 text-white/50 hover:text-white"}`}>{l}</button>
            ))}
            <span className="ml-auto text-xs font-mono text-white/40 self-center">{filtered.length} results</span>
          </div>
            ...
            {filtered.map((a) => (
              <article key={a.slug} className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] hover:border-emerald-500/30 transition">
                <div className="relative aspect-[16/10] overflow-hidden bg-black/40">
                  ...
                  <span className="absolute top-3 left-3 text-[9px] font-mono uppercase tracking-widest text-emerald-300 border border-emerald-500/40 bg-black/50 rounded px-1.5 py-0.5">{a.category}</span>
                  {a.featured && <span className="absolute top-3 right-3 text-[9px] font-mono uppercase tracking-widest text-white/80 border border-white/30 bg-black/50 rounded px-1.5 py-0.5">Featured</span>}
                </div>
                <div className="p-5">
                  <div className="text-[10px] font-mono uppercase tracking-widest text-white/40">{a.section} · {a.level}</div>
                  <h3 className="mt-2 text-lg font-semibold tracking-tight group-hover:text-emerald-300 transition leading-snug">{a.title}</h3>
                  <p className="mt-2 text-sm text-white/60 leading-relaxed line-clamp-2">{a.excerpt}</p>
                  <div className="mt-5 flex items-center justify-between text-xs">
                    <span className="inline-flex items-center gap-1 text-white/40"><Clock className="h-3.5 w-3.5" /> {a.readMin} min read</span>
      ...
      <section id="academy" className="border-b border-white/5 py-20">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-[10px] font-mono uppercase tracking-widest text-emerald-400">Sandwich Panel Academy</div>
          <h2 className="mt-2 text-3xl font-semibold tracking-tight">Learning Center</h2>
          <p className="mt-3 text-white/60 max-w-2xl">Structured courses at three levels — from panel foundations to factory investment engineering.</p>
              ...
              <div key={c.title} className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] hover:border-emerald-500/30 transition">
                <div className="relative aspect-[16/10] overflow-hidden">
                  ...
                  <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-[10px] font-mono uppercase tracking-widest">
                    <span className="text-emerald-300 border border-emerald-500/40 bg-black/50 rounded px-1.5 py-0.5">{c.level}</span>
                    <span className="text-white/70 border border-white/20 bg-black/50 rounded px-1.5 py-0.5">{c.tag}</span>
                  </div>
                </div>
                <div className="p-5">
                  <h3 className="text-lg font-semibold tracking-tight group-hover:text-emerald-300 transition">{c.title}</h3>
          ...
          <div className="mt-16 grid grid-cols-[minmax(0,1fr)] gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                ...
                {VIDEOS.map((v) => (
                  <div key={v.title} className="group flex items-center gap-4 rounded-xl border border-white/10 bg-white/[0.02] p-3 hover:border-emerald-500/30 transition">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium truncate group-hover:text-emerald-300 transition">{v.title}</div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
              ...
              <div className="mt-5 space-y-3">
                {DOWNLOADS.map((d) => (
                  <Link key={d.title} to="/download-center" className="group flex items-center gap-4 rounded-xl border border-white/10 bg-white/[0.02] p-3 hover:border-emerald-500/30 transition">
                    <div className="relative h-16 w-24 shrink-0 overflow-hidden rounded-lg">
                      <img src={d.cover} alt="" loading="lazy" className="h-full w-full object-cover" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium truncate group-hover:text-emerald-300 transition">{d.title}</div>
                ...
                <Link to="/download-center" className="block text-center text-xs font-mono uppercase tracking-widest text-emerald-400 hover:text-emerald-300 py-3">
      ...
      <section id="faq" className="border-b border-white/5 py-20">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-[10px] font-mono uppercase tracking-widest text-emerald-400">FAQ Center</div>
          <h2 className="mt-2 text-3xl font-semibold tracking-tight">Answers from our engineers</h2>
          <p className="mt-3 text-white/60 max-w-2xl">Six categories — technical, production, investment, engineering, installation and maintenance.</p>

          <div className="mt-10 grid md:grid-cols-2 gap-6">
            {FAQS.map((g) => {
              const Icon = g.icon;
              return (
                <div key={g.group} className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
      ...
      <section className="border-b border-white/5 py-20 bg-gradient-to-b from-emerald-500/[0.04] to-transparent">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-[10px] font-mono uppercase tracking-widest text-emerald-400">Related</div>
          <h2 className="mt-2 text-3xl font-semibold tracking-tight">Keep going — related content & tools</h2>

          <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { icon: Calculator, title: "Engineering Tools", desc: "20 real-time calculators.", href: "/engineering-tools" },
              { icon: Download,   title: "Download Center",   desc: "CAD, BIM, certifications, manuals.", href: "/download-center" },
              { icon: Snowflake,  title: "PIR vs Rock Wool",  desc: "The full comparison page.", href: "/pir-vs-rock-wool" },
              { icon: Sparkles,   title: "AI Engineer",       desc: "Ask any panel or factory question.", href: "/ai-assistant" },
            ].map((r) => {
              const Icon = r.icon;
              return (
                <Link key={r.title} to={r.href} className="group rounded-2xl border border-white/10 bg-white/[0.02] hover:border-emerald-500/30 p-5 transition">
                  <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400"><Icon className="h-5 w-5" /></div>
                  <div className="mt-4 font-semibold group-hover:text-emerald-300 transition">{r.title}</div>
                  <div className="mt-1 text-xs text-white/50">{r.desc}</div>
                  <div className="mt-4 inline-flex items-center gap-1 text-xs font-medium text-emerald-400">Open <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-1" /></div>
                </Link>
              );
            })}
          </div>
      ...
      <section className="py-24">
        <div className="mx-auto max-w-5xl px-6 text-center">
          ...
          <h2 className="mt-6 text-4xl md:text-5xl font-semibold tracking-tight">Talk to a NEVO engineer</h2>
          <p className="mt-4 text-white/60 max-w-2xl mx-auto">Have a project or a question our library doesn't cover yet? A senior industrial engineer will reply within 24 hours.</p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            ...
            <Button asChild variant="outline" className="border-white/15 bg-white/[0.03] h-12 px-6">
              <Link to="/download-center"><Download className="h-4 w-4 mr-2" /> Download Engineering Guide</Link>
            </Button>
            <Button asChild variant="ghost" className="text-white/70 hover:text-white h-12 px-6">
              <a href="#academy"><PlayCircle className="h-4 w-4 mr-2" /> Watch Training</a>
            </Button>
          </div>
