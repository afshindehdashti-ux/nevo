...
const ROLES = [
  {
    title: "Senior Process Engineer — PIR Lines",
    team: "Engineering",
    location: "Dubai, UAE",
    type: "Full-time",
    level: "Senior",
  ...
  {
    title: "Project Manager — Factory Delivery",
  ...
  {
    title: "International Sales Director — GCC",
    team: "Sales",
    location: "Dubai, UAE",
    type: "Full-time",
    level: "Director",
  ...
  {
    title: "Electrical Engineer — Panel Boards",
    team: "Electrical",
    location: "Istanbul, TR",
    type: "Full-time",
    level: "Mid",
  ...
  {
    title: "Marketing Manager — Industrial B2B",
    team: "Marketing",
    location: "Dubai, UAE",
    type: "Full-time",
    level: "Mid",
  ...
  {
    title: "Graduate Engineer — Class of 2026",
    team: "Graduate",
    location: "Multiple",
    type: "Program",
    level: "Entry",
...
const BENEFITS = [
  {
    ...
    body: "Premium medical insurance, mental health support and on-site clinics.",
  },
  {
    icon: GraduationCap,
    title: "Learning Budget",
    body: "Annual budget for certifications (PMP, PE, PLC vendor training).",
  },
  {
    icon: Globe2,
    title: "Global Mobility",
    body: "Rotation opportunities between Dubai, Germany, Türkiye and Oman.",
  },
  {
    icon: Rocket,
    title: "Stock & Bonus",
    body: "Performance bonus and long-term incentive plan for senior engineers.",
  },
  {
    icon: Users,
    title: "Family Support",
    body: "Parental leave, relocation assistance and international schooling stipend.",
  },
  {
    icon: Clock,
    title: "Balanced Schedules",
    body: "Hybrid where possible, protected quiet hours, generous PTO.",
  },
];
...
function CareersPage() {
  ...
  async function handleApplication(e: React.FormEvent<HTMLFormElement>) {
    ...
    const tick = () => {
      ...
      toast.loading(
        cvName ? `Uploading CV — ${cvName} · ${pct}%` : `Submitting application… ${pct}%`,
        {
          id: loadingId,
          description:
            uploadedMb && totalMb
    ...
    try {
      ...
      setProgress(100);
      toast.loading(cvName ? `CV uploaded — ${cvName} · 100%` : "Application submitted · 100%", {
        id: loadingId,
        description: "Finalising your confirmation…",
      });
    ...
    <>
        ...
        <div className="container-wide relative py-32 md:py-40">
          ...
          >
            ...
            <p className="mt-6 max-w-2xl text-lg text-white/70">
              Senior engineers, ambitious graduates, sharp commercial minds — we hire people who
              want to build industrial infrastructure the world will still rely on in 2050.
            </p>
            ...
            <div className="mt-16 grid max-w-2xl grid-cols-3 gap-8 border-t border-white/10 pt-8">
              <div>
                <div className="text-4xl font-semibold text-emerald-400">61</div>
                <div className="mt-1 text-xs uppercase tracking-widest text-white/50">
                  Open Roles
                </div>
              </div>
              <div>
                <div className="text-4xl font-semibold text-emerald-400">18</div>
                <div className="mt-1 text-xs uppercase tracking-widest text-white/50">
                  Nationalities
                </div>
              </div>
              <div>
                <div className="text-4xl font-semibold text-emerald-400">4.8</div>
                <div className="mt-1 text-xs uppercase tracking-widest text-white/50">
                  Glassdoor Score
                </div>
              </div>
      ...
      <Section id="open-roles">
        <SectionHeader
          ...
          title="Roles hiring now"
          aside={
            <Button variant="outline" asChild>
              <a href="#upload-cv">Speculative Application</a>
            </Button>
          }
        />
      ...
      <Section id="upload-cv">
        <div className="mx-auto max-w-3xl rounded-2xl border border-border bg-background p-8 md:p-12">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Even if a matching role isn't listed today — our talent team reviews every application.
          </p>
          <form ref={cvFormRef} className="mt-8 grid gap-4" onSubmit={handleApplication} noValidate>
            ...
            <label className="flex cursor-pointer items-center justify-between rounded-md border border-dashed border-input bg-muted/40 px-4 py-6 text-sm text-muted-foreground hover:bg-muted">
              ...
              <span className="text-xs">
                {cvName ? (busy ? "Uploading…" : "Change") : "Click to browse"}
              </span>
              <input
                name="cv"
                type="file"
                ...
                onChange={(e) => {
                  ...
                  if (f.size > 8 * 1024 * 1024) {
                    ...
                    import("sonner").then(({ toast }) =>
                      toast.error("File too large", { description: "CV must be under 8 MB." }),
                    );
                    return;
                  }
                  setCvName(f.name);
            ...
            <Button type="submit" size="lg" disabled={busy}>
              {busy ? (
                <>
                  ...
                  {cvName ? `Uploading CV… ${progress}%` : `Submitting… ${progress}%`}
                </>
              ) : (
                <>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-1 text-[11px] text-muted-foreground">
                      Failed at {uploadError.at.toLocaleTimeString()} · Attempt{" "}
                      {uploadError.attempt}
      ...
      <Section className="bg-graphite text-white">
        <div className="mx-auto flex max-w-4xl flex-col items-center gap-6 text-center">
          ...
          <p className="max-w-2xl text-sm text-white/70">
            Requesting a factory, panel line or sandwich panel quotation? Our sales engineers
