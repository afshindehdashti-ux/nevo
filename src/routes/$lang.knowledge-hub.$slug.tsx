          ...
          children: JSON.stringify({
            ...
            itemListElement: [
              { "@type": "ListItem", position: 1, name: "Home", item: SITE.url },
              {
                ...
                position: 2,
                name: "Knowledge Hub",
                item: `${SITE.url}/${params.lang}/knowledge-hub`,
              },
      ...
      <header className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="relative mx-auto max-w-4xl px-6 pt-24 pb-14">
          ...
          >
            ...
            <Link to="/knowledge-hub" className="hover:text-emerald-400">
              Knowledge Hub
            </Link>
            <ChevronRight className="h-3 w-3" />
            <span className="text-white/70 normal-case tracking-normal font-sans">
              {article.category}
            </span>
          </nav>
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[10px] font-mono uppercase tracking-widest text-emerald-300 border border-emerald-500/40 bg-emerald-500/10 rounded px-2 py-1">
              {article.category}
            </span>
            <span className="text-[10px] font-mono uppercase tracking-widest text-white/60 border border-white/15 rounded px-2 py-1">
              {article.section}
            </span>
            <span className="text-[10px] font-mono uppercase tracking-widest text-white/60 border border-white/15 rounded px-2 py-1">
              {article.level}
          ...
          <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-xs text-white/50">
            ...
            <span className="inline-flex items-center gap-1.5">
              <Clock className="h-3.5 w-3.5" /> {article.readMin} min read
            </span>
          </div>
        </div>
      </header>
        ...
        <div className="mt-16 rounded-2xl border border-white/10 bg-white/[0.02] p-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <div className="text-xs text-white/50 mt-1">
              A NEVO senior engineer will respond within one working day.
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <Button asChild variant="outline" className="border-white/15">
              <Link to="/download-center">
                <Download className="h-4 w-4 mr-2" /> Guides
              </Link>
            </Button>
          </div>
        </div>
      ...
      <section className="border-t border-white/5 py-16 bg-gradient-to-b from-transparent to-emerald-500/[0.03]">
        <div className="mx-auto max-w-6xl px-6">
          <div className="flex items-end justify-between gap-4">
            ...
            <Link
              to="/knowledge-hub"
              className="inline-flex items-center gap-1 text-xs font-mono uppercase tracking-widest text-white/60 hover:text-emerald-400"
            >
          ...
          <div className="mt-8 grid md:grid-cols-3 gap-5">
            {rel.map((r) => (
              <Link
                ...
                params={{ slug: r.slug }}
                className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] hover:border-emerald-500/30 transition"
              >
      ...
      <section className="py-20 border-t border-white/5">
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          <p className="mt-4 text-white/60 max-w-2xl mx-auto">
            500+ technical articles, 24 academy courses and 120+ downloads — written by NEVO
            engineers.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            ...
            <Button asChild variant="outline" className="border-white/15 bg-white/[0.03] h-12 px-6">
              <Link to="/ai-assistant">
                <Sparkles className="h-4 w-4 mr-2" /> Ask the AI Engineer
              </Link>
            </Button>
            <Button asChild variant="ghost" className="text-white/70 hover:text-white h-12 px-6">
              <a href="mailto:engineers@nevo-industrial.com">
