...
function classify(result, expectJson = false) {
  ...
  if (result.status === 307 || result.status === 308) return `${result.status} redirect — old build still cached`;
...
async function verifyOnce(baseUrl) {
  const [root, health] = await Promise.all([probe(baseUrl), probe(baseUrl.replace(/\/$/, '') + HEALTH_PATH)]);
...
async function main() {
  ...
  for (let i = 1; i <= attempts; i++) {
    ...
    if (r.ok) {
      console.log(`[verify-publish] ✅ attempt ${i}/${attempts} — root=${r.root.status} health=${r.health.status} (${dt}ms)`);
      console.log(`[verify-publish] health body: ${r.health.body.trim()}`);
      console.log('[verify-publish] production is live and responding.');
      process.exit(0);
    }
    const msg = [r.rootErr && `root: ${r.rootErr}`, r.healthErr && `health: ${r.healthErr}`].filter(Boolean).join(' | ');
