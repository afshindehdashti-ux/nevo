    ...
    if summary_path and any_artifact:
        ...
        with open(summary_path, "a", encoding="utf-8") as fh:
            ...
            fh.write(
                ...
                'aria-pressed="false" '
                'aria-label="Sort direction: ascending. Activate to sort descending.">Asc</button></p>\n\n'
                ...
                'aria-pressed="false" '
                'aria-label="Item sort direction: ascending. Activate to sort descending.">Asc</button></p>\n\n'
                ...
                '} '
                 ...
                 'const sortLabel = document.querySelector("#artifact-sort option[value=\\"" + key + "\\"]"); '
                 'announceArtifact("Groups sorted by " + (sortLabel ? sortLabel.textContent : key) + ", " + (dir === "asc" ? "ascending" : "descending")); '
                 '} '
                 'function _syncSortDirButton(btn, dir, kind) { '
                 'const label = dir === "asc" ? "ascending" : "descending"; '
                 'const opposite = dir === "asc" ? "descending" : "ascending"; '
                 'btn.dataset.dir = dir; '
                 ...
                 '_syncSortDirButton(btn, next, "Sort direction"); '
                 'sortArtifactGroups(); '
                 'announceArtifact("Groups sorted " + (next === "asc" ? "ascending" : "descending")); '
                 '} '
                 ...
                 'if (dir === "asc" || dir === "desc") { _syncSortDirButton(btn, dir, "Sort direction"); } '
                 ...
                 'const isortLabel = document.querySelector("#artifact-item-sort option[value=\\"" + key + "\\"]"); '
                 'announceArtifact("Items sorted by " + (isortLabel ? isortLabel.textContent : key) + ", " + (dir === "asc" ? "ascending" : "descending")); '
                 '} '
                 'function toggleItemSortDirection() { '
                 'const btn = document.getElementById("artifact-item-sort-dir"); '
                 'const current = btn.dataset.dir || "asc"; '
                 'const next = current === "asc" ? "desc" : "asc"; '
                 '_syncSortDirButton(btn, next, "Item sort direction"); '
                 'sortArtifactItems(); '
                 'announceArtifact("Items sorted " + (next === "asc" ? "ascending" : "descending")); '
                 '} '
                 ...
                 'if (key && validKeys.indexOf(key) !== -1) sel.value = key; '
                 'if (dir === "asc" || dir === "desc") { _syncSortDirButton(btn, dir, "Item sort direction"); } '
                ...
                'g.style.display = visible > 0 ? "" : "none"; '
                 ...
                 'if (document.readyState === "loading") { document.addEventListener("DOMContentLoaded", restoreArtifactFilters); } else { restoreArtifactFilters(); } '
                   ...
                   'const modeLabel = toggle.checked ? "absolute URLs" : "file paths"; '
                   'document.querySelectorAll(".artifact-url-mode").forEach(el => { '
                   'el.textContent = modeLabel; '
                   '}); '
                   'showCopyToast("Copy mode set to " + modeLabel); '
                   '} '
                   'persistArtifactFilters(); '
                   '} '
                ...
                'const bulkBtns = document.querySelectorAll(".artifact-bulk-action[data-requires-selection=\\"true\\"]"); '
                'const wasDisabled = window.__artifactBulkDisabled; '
                'const isDisabled = n === 0; '
                'bulkBtns.forEach(btn => { '
                'btn.setAttribute("aria-disabled", isDisabled ? "true" : "false"); '
                'if (isDisabled) { btn.setAttribute("tabindex", "0"); } else { btn.removeAttribute("tabindex"); } '
                '}); '
                'window.__artifactBulkDisabled = isDisabled; '
                'if (wasDisabled !== undefined && wasDisabled !== isDisabled) { '
                'announceArtifact(isDisabled ? "Bulk actions disabled — no artifacts selected" : "Bulk actions available for " + n + " selected artifact" + (n === 1 ? "" : "s")); '
                '} '
                'if (window.__artifactSelAnnounceTimer) { clearTimeout(window.__artifactSelAnnounceTimer); } '
                'window.__artifactSelAnnounceTimer = setTimeout(function() { '
                'const liveEl = document.getElementById("artifact-selection-live"); '
                'if (!liveEl) { return; } '
                'const msg = n === 0 ? "No artifacts selected" : (total > 0 ? (n + " of " + total + " artifact" + (total === 1 ? "" : "s") + " selected") : (n + " artifact" + (n === 1 ? "" : "s") + " selected")); '
                'if (liveEl.textContent === msg) { liveEl.textContent = ""; } '
            ...
            if all_paths:
                ...
                bulk_buf.write(
                    ...
                    '<span id="artifact-selection-live" class="visually-hidden" '
                    ...
                    '</span> '
                    '<span id="artifact-select-desc" class="visually-hidden">'
                    'Toggles inclusion of this artifact in the current selection for copy or export actions.'
                    '</span> '
                    ...
                    '<button type="button" class="artifact-bulk-action" data-requires-selection="true" aria-label="Clear artifact selection" '
                    'aria-keyshortcuts="Escape" '
                    'onclick="clearArtifactSelection()">Clear selection</button> '
                    '<button type="button" class="artifact-bulk-action" data-requires-selection="true" aria-label="Copy selected artifact links as Markdown" '
                    'onclick="copySelectedArtifacts(\'markdown\')">Copy selected (Markdown)</button> '
                    '<button type="button" class="artifact-bulk-action" data-requires-selection="true" aria-label="Copy selected artifact links as JSON" '
                    'onclick="copySelectedArtifacts(\'json\')">Copy selected (JSON)</button> '
                    '<button type="button" class="artifact-bulk-action" data-requires-selection="true" aria-label="Copy selected artifact links as CSV" '
                    'onclick="copySelectedArtifacts(\'csv\')">Copy selected (CSV)</button> '
                    '<button type="button" class="artifact-bulk-action" data-requires-selection="true" aria-label="Export manifest for selected artifacts as CSV" '
                    'onclick="exportSelectedManifest(\'csv\')">Export selected manifest (CSV)</button> '
                    '<button type="button" class="artifact-bulk-action" data-requires-selection="true" aria-label="Export manifest for selected artifacts as JSON" '
                    'onclick="exportSelectedManifest(\'json\')">Export selected manifest (JSON)</button> '
                    '<button type="button" class="artifact-bulk-action" data-requires-selection="true" aria-label="Download selected artifact files" '
                    ...
                    if is_heatmap_group:
                        if exists:
                            ...
                            fh.write(
                                ...
                                f'<label class="artifact-select-label">'
                                f'<input type="checkbox" class="artifact-select" aria-checked="false" aria-label="Select {html.escape(label, quote=True)}" aria-describedby="artifact-select-desc"><span class="visually-hidden">Select {html.escape(label)}</span></label>'
